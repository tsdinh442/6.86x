...
def run_softmax_on_MNIST(temp_parameter=1):
    ...
    update_train_y, update_test_y = update_y(train_y, test_y)
    #new_theta, _ = softmax_regression(train_x, update_train_y, temp_parameter, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
    #test_error = compute_test_error(test_x, update_test_y, new_theta, temp_parameter)

    test_error = compute_test_error_mod3(test_x, update_test_y, theta, temp_parameter)
    return test_error
...
def run_softmax_on_MNIST_PCA(temp_parameter=1):
    ...
    return test_error
print('PCA - softmax test_error=', run_softmax_on_MNIST_PCA(temp_parameter=1))
...
def run_softmax_on_MNIST_cubic(temp_parameter=1):
    ...
    return test_error
print('Cubic - softmax test_error=', run_softmax_on_MNIST_cubic(temp_parameter=1))

def poly_SVM():
    svm = SVC(kernel='poly', random_state=0, degree=3)
    svm.fit(train_pca10, train_y)
    pred = svm.predict(test_pca10)

    return compute_test_error_svm(pred, test_y)

print('Poly - SVM Error=', poly_SVM())
