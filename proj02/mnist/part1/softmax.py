...
def compute_probabilities(X, theta, temp_parameter):
    ...
    """
    linear = np.dot(theta, np.transpose(X)) / temp_parameter
    c = linear.max(axis=0)
    exp = np.exp(linear - c)
    h = (1 / np.sum(exp, axis=0)) * exp
    return h
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    log = np.where(prob != 0, np.log(prob), 0)
    loss = np.sum(np.sum(M * log, axis=0)) / num_examples
